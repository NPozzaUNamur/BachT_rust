...
impl<'a> BachTStore<'a> {
    ...
    pub(crate) fn tell(&mut self, token: &'a str) -> bool {
    ...
    pub(crate) fn ask(&mut self, token: &'a str) -> bool {
    ...
    ///
    /// **@returns** - true if the token is in the store, false otherwise
    pub(crate) fn get(&mut self, token: &'a str) -> bool {
        let mut res = false;
        self.the_store.entry(token).and_modify(|nbr_occurrence| {
            if *nbr_occurrence > 0 {
                *nbr_occurrence -= 1;
    ...
    pub(crate) fn nask(&mut self, token: &str) -> bool {
        if !self.the_store.contains_key(token) {
            true
        } else {
            self.the_store.get(token).unwrap() <= &0
